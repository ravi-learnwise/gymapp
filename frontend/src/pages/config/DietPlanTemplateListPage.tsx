...
import { OBJECTIVE_LABELS, type DietPlanTemplateListItem } from '../../types/diet-plan';

export default function DietPlanTemplateListPage() {
  const [items, setItems] = useState<DietPlanTemplateListItem[]>([]);
  const [search, setSearch] = useState('');
  const [deactivateId, setDeactivateId] = useState<string | null>(null);
  const [message, setMessage] = useState('');

  const load = () => api<DietPlanTemplateListItem[]>('/diet-plan-templates').then(setItems);
  ...
  const deactivate = async () => {
    ...
    try {
      await api(`/diet-plan-templates/${deactivateId}/deactivate`, {
        method: 'POST',
        body: '{}',
      });
  ...
  return (
    <div className="max-w-5xl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-2xl">Diet Plan Templates</h2>
          <p className="mt-1 text-sm text-ink-secondary">
            Gym-level weekly diet recipes. Trainers can apply these when assigning a member plan.
          </p>
        </div>
        <Link to="/config/diet-templates/new" className="btn btn-primary">
          New template
        </Link>
      </div>
          ...
          <ListTableBody>
            {filtered.map((t) => (
              <tr key={t.id}>
                ...
                <td className="text-sm text-ink-secondary">{OBJECTIVE_LABELS[t.objective]}</td>
                <td>{t.isActive ? 'Active' : 'Inactive'}</td>
                <td>
                  <div className="flex gap-2">
                    <IconActionLink variant="edit" to={`/config/diet-templates/${t.id}/edit`} />
                    {t.isActive && (
            ...
            {!filtered.length && <ListTableEmpty colSpan={4} message="No diet plan templates" />}
          </ListTableBody>
        </ListTable>
      </div>
